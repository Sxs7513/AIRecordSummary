...
`run_example()` 只是一个无数据库的演示运行器；它直接遍历 `example_pipeline` 的拓扑顺序，不手写节点执行顺序。生产业务域负责把自己的 Definition 和 stage registry 组合起来；`pipeline` 本身不包含具体业务流水线。
...
1. 实现 `name`、`version`、`retry_policy` 和 `async run()`。
2. 在应用启动时通过 `StageRegistry.register()` 显式注册实例。
3. 在 `PipelineDefinition` 中用同一组 `name/version` 声明节点与依赖。
4. 业务域自己的持久化与协调器将 Definition 映射为运行记录；Stage 通过内部 HTTP Compute Worker 调用需要隔离的原子 CPU/GPU 操作。
