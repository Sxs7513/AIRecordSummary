...
from contracts import TrainingManifest, TrainingPreset
...
def test_training_preset_uses_single_process_data_loading_by_default() -> None:
    assert TrainingPreset.from_dict({}).num_workers == 0


def test_training_preset_rejects_negative_worker_count() -> None:
    with pytest.raises(ValueError, match="worker"):
        TrainingPreset.from_dict({"num_workers": -1})
...
def test_manifest_loads_paths_and_preset(tmp_path: Path) -> None:
    ...
    assert manifest.preset.num_workers == 0
