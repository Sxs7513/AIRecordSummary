...
from l2_core.application.processing_queue import ProcessingQueueUnavailableError
...
from l2_core.audio_processing.stages.summary.regeneration import RecordingSummaryNotReadyError, RecordingSummaryQueueUnavailableError
...
async def create_recording(
    ...
    except ProcessingQueueUnavailableError as error:
...
async def delete_recording(recording_id: UUID, service: RecordingServiceDependency, user: CurrentUserDependency) -> None:
    """Permanently remove one recording and its associated files and results."""
    try:
        result = service.delete_recording(user, recording_id)
...
async def retry_recording_embedding_index(
    recording_id: UUID,
    service: RecordingServiceDependency,
    user: CurrentUserDependency,
    ...
    try:
        result = service.retry_embedding_indexing(user, recording_id)
        stage_run_id = await result if isawaitable(result) else result
    except RecordingAccessDeniedError as error:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Recording access denied") from error
...
async def regenerate_recording_summary(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(error)) from error
    except RecordingSummaryQueueUnavailableError as error:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail=str(error)) from error
    except RecordingAccessDeniedError as error:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Recording access denied") from error
