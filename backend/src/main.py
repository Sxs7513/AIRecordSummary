...
from audio_processing.registry import build_recording_stage_registry, build_recording_summary_stage
...
from pipeline.runtime.coordinator import PipelineCoordinator, run_pipeline_coordinator
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None]:
    ...
    if app.state.settings.pipeline_embedded_workers_enabled:
...
def create_app(settings: Settings | None = None) -> FastAPI:
    ...
    app.include_router(api_router, prefix=configured_settings.api_prefix)
    return app
