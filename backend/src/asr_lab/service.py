...
from evaluation.contracts import ApprovedAnnotation, DatasetVersionPreview
    ...
    def create_training_run(
        ...
        *,
        ...
        with self._engine.begin() as connection:
            ...
            self._model_row(connection, user, base_model_version_id)
            row = connection.execute(
                text(
                    """
        ...
        with self._engine.begin() as connection:
            connection.execute(
                text(
                    ...
                    values (
                        :workspace_id, 'qwen3_asr', 'Qwen3-ASR-1.7B', 'base',
                        'Qwen/Qwen3-ASR-1.7B', 'approved',
                        '{"provider":"qwen_asr"}'::jsonb,
                        '{"source":"built_in"}'::jsonb,
                        :user_id
                    )
