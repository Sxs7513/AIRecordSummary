...
录音处理完成后，文字记录会按说话人与时间范围展示。**图中的每段文字记录都可以点击**；点击后播放器会直接跳转到对应的录音时间点，方便结合原音核对转写内容和上下文。

> 下图使用的演示录音来自网络公开资料，不包含私人会议或内部录音。

![可点击并跳转到对应录音时间的文字记录](./pics/audio-recording.png)

### 带录音引用的 RAG 问答

用户可以直接针对录音内容提问。系统从相关录音中召回证据并生成回答，同时在答案中标注引用；点击引用可以查看对应的录音片段和时间位置，便于回到原音核对。

> 下图使用的演示录音同样来自网络公开资料。

![带录音引用的 RAG 问答](./pics/rag-asr.png)
