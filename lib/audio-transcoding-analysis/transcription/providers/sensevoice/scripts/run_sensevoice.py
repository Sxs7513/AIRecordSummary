...
def main() -> int:
    ...
    with contextlib.redirect_stdout(sys.stderr):
        progress("import", "加载 SenseVoice 依赖", 5)
        try:
        ...
        progress("load_model", f"加载 SenseVoice 模型 {args.model}", 25)
        model = AutoModel(model=args.model, trust_remote_code=True, hub="hf")
    ...
    try:
        ...
        for index, (start_ms, end_ms) in enumerate(vad_segments):
            ...
            try:
                ...
                print(f"Skipping SenseVoice segment {start_ms}-{end_ms}: {exc}", file=sys.stderr)
    finally:
    ...
    progress("done", "SenseVoice 转写完成", 100)
