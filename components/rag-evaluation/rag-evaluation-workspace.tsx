    ...
    <section className="panel grid">
      ...
      {detail && (
        <>
          ...
          <div className="stats rag-eval-stats">
            <MetricCard label="Answer Score" metric={metric(finalMetrics, "answer_score")} fractionDigits={0} suffix=" / 100" />
            <MetricCard label="Key Point 正确率" metric={metric(finalMetrics, "key_point_correctness")} percent />
...
function MetricCard({ label, metric: value, percent = false, fractionDigits, suffix = "" }: { label: string; metric: number | null; percent?: boolean; fractionDigits?: number; suffix?: string }) {
  return <div className="stat card"><span className="subtle">{label}</span><strong>{formatMetric(value, percent, fractionDigits)}{value === null ? "" : suffix}</strong></div>;
}
...
function formatMetric(value: number | null, percent = false, fractionDigits?: number): string {
  if (value === null || Number.isNaN(value)) return "—";
  const digits = fractionDigits ?? (percent ? 1 : 3);
